...
const ForgetPassword = async (req, res) => {
  ...
  try {
    ...
    console.error(error);
    res.send({ status: "error", data: "Error in forget password" });
  }
};
...
const SaveChanges = async (req, res) => {
  console.log("SaveChanges function is calling.");
  // const {fname,lname,email,address,scnumber,contactNo,degreeProgram,academicLevel,year,department,faculty} = req.body;
  ...
  try {
    ...
    await User.updateOne(
      { email },
      {
        $set: {
          ...
          // faculty:faculty,
          // photoUrl:cancelChanges ? tempPhotoUrl : updatedUserData.photoUrl,
        },
      }
    );
    ...
    // ) {
    //     if (!(await bcrypt.compare(updatedUserData.currentPassword, user.password))) {
    //         return res.json({ status: 'error', message: 'Invalid current password' });
    //     }

    //     if (
    //         updatedUserData.newPassword !== updatedUserData.confirmPassword
    //     ) {
    //         return res.json({
    //             status: 'error',
    ...
    //     const encryptedPassword = await bcrypt.hash(updatedUserData.newPassword, 10);
